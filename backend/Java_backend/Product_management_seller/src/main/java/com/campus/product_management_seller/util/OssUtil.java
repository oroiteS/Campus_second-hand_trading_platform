package com.campus.product_management_seller.util;
...
public class OssUtil {
     ...
     * 上传商品图片到OSS
     * 
     * @param files 商品图片文件列表
     * @param commodityId 商品ID
     * @return 上传成功的图片URL列表
     * @throws IOException 上传异常
     */
    public List<String> uploadCommodityImages(List<MultipartFile> files, String commodityId) throws IOException {
        ...
        for (MultipartFile file : files) {
            ...
            // 生成文件key
            String fileKey = generateCommodityImageKey(file, commodityId);
            ...
            try {
                ...
                // 生成公共访问URL
                String imageUrl = generatePublicUrl(fileKey);
                imageUrls.add(imageUrl);
                
            } catch (Exception e) {
                // 如果某个文件上传失败，删除已上传的文件
                for (String uploadedUrl : imageUrls) {
                    String uploadedKey = extractFileKeyFromUrl(uploadedUrl);
                    deleteFile(uploadedKey);
                }
                throw new IOException("上传商品图片到OSS失败: " + e.getMessage(), e);
            }
        }
     ...
     * 生成商品图片文件key
     */
    private String generateCommodityImageKey(MultipartFile file, String commodityId) {
        String originalFilename = file.getOriginalFilename();
        String extension = getFileExtension(originalFilename);
        ...
        // 组合文件key：commodity/商品ID_时间戳_随机字符串.扩展名
        return "commodity/" + commodityId + "_" + timestamp + "_" + randomStr + extension;
    }
