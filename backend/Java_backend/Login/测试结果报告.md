...
**建议解决方案**:
1. **PowerShell编码**: 设置 `[Console]::OutputEncoding = [System.Text.Encoding]::UTF8`
2. **MySQL客户端**: 使用 `SET NAMES utf8mb4;` 设置字符集
3. **应用配置**: 确保 `application.yml` 中数据库连接使用UTF-8编码
